...
#create route
@router.post("/recipes")
def route_create_recipe(title: str, author_name:str, ingredients:str, cook_time:str, db: Session = Depends(get_db_session)):
    return create_recipe(db, title, author_name, ingredients, cook_time)

#read route
@router.get("/recipe/{recipe_id}")
def route_get_recipe(recipe_id: int, db: Session =  Depends(get_db_session)):
...
def route_delete_recipe(recipe_id: int, db: Session = Depends(get_db_session)):
